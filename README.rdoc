...
If you would like to install a crontab with a different name just give that
name as a string example:

task :production do
  role :web,                  'example.com', :crontab => 'web_crontab'
  role :app,                  'example.com', :crontab => 'app_crontab'
  role :db,                   'example.com', :primary => true
end
